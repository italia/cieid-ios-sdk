...
void ByteArray::copy(const ByteArray &src, size_t start) {
    ...
    if(src._size > 0)
        memcpy(_data + start, src._data, src._size);
...
void ByteArray::rightcopy(const ByteArray &src, size_t end) {
    ...
    memcpy(_data + _size - end - src._size, src._data, src._size);
...
void ByteDynArray::resize(size_t size, bool bKeepData) {
	if (!bKeepData) {
        ...
        if (dwMinSize> 0)
            memcpy(pbtNewData, _data, dwMinSize);
