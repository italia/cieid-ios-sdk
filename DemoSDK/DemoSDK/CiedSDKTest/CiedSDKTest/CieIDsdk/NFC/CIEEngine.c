        ...
        const unsigned char *tbs, size_t tbslen)
{
    ...
    if(!makeDigestInfo(md_type, tbs, tbslen, digestinfo, &digestinfolen))
        return 0;
        
    ret = sign(digestinfo, digestinfolen, sig, siglen) & 0x0000FFFF;
    if(ret != CIE_CMD_OK)
    {
        cie_error = ret;
