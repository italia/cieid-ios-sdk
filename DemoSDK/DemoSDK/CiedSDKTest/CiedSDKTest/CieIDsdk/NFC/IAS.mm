...
#define CIE_KEY_Sign_ID_FOR_PUK_RECOVER 0x83
...
void IAS::ReadATR(ByteDynArray &data) {
    init_func
    readfile(0x2f01, data);
    exit_func
}
...
//PUK RECOVER
ByteDynArray IAS::GetModuleFromPubKey(ByteDynArray &data){
    
    init_func
    CASNParser parser;
    parser.Parse(data);
    ByteDynArray module = parser.tags[0]->tags[0]->content;
...
//PUK RECOVER
ByteDynArray IAS::GetExpFromPubKey(ByteDynArray &data){
    
    init_func
    CASNParser parser;
    parser.Parse(data);
    ByteDynArray exp = parser.tags[0]->tags[1]->content;
...
//PUK RECOVER
void IAS::SignIntAuth(ByteArray &data, ByteDynArray &signedData) {
    init_func
    ByteDynArray resp;
    uint8_t SetKey[] = { 0x00, 0x22, 0x41, 0xA4 };
    uint8_t val02 = 2;
    uint8_t keyId = CIE_KEY_Sign_ID_FOR_PUK_RECOVER;
    StatusWord sw;
    ByteArray val02Ba = VarToByteArray(val02);
    ByteArray keyIdBa = VarToByteArray(keyId);
    if ((sw = SendAPDU(VarToByteArray(SetKey), ASN1Tag(0x80, val02Ba).append(ASN1Tag(0x84, keyIdBa)), resp)) != 0x9000)
        throw scard_error(sw);
    
    uint8_t Sign[] = { 0x00, 0x88, 0x00, 0x00 };
    if ((sw = SendAPDU(VarToByteArray(Sign), data, signedData)) != 0x9000)
        throw scard_error(sw);
}
...
//		dh_prKey.random();
//        printf("\n\ndhpubKey: %s", dumpHexData(dh_prKey).c_str());
//        printf("\n\dh_q: %s\n\n", dumpHexData(dh_q).c_str());
//        printf("%x, %x\n", dh_prKey[0], dh_q[0]);
//        i++;
//	} while (dh_q[0] > 0 && dh_q[0] < dh_prKey[0] && i < 1000);
...
//        printf("\n\dh_q: %s\n\n", dumpHexData(dh_q).c_str());
        i++;
    } while (dh_q[0] < dh_prKey[0] && i < 1000);
    
	// dh_prKey deve essere dispari
	dh_prKey.right(1)[0] |= 1;

    ByteDynArray dhg(dh_p.size());//WAS dh_g.size - SM Data object incorrect error
    dhg.fill(0);
	dhg.rightcopy(dh_g);
...
//    std::string dmp;
//    printf("smMac: ");
//    printf("%s", dumpHexData(smMac, dmp).c_str());
//    printf("\nrespMac: ");
//    printf("%s", dumpHexData(respMac, dmp).c_str());

//    if(!(smMac == respMac))
//        printf("warning: MAC non corrispondenti\n");
//        throw logged_error("Mac non corrispondenti");//,"Errore nel checksum della risposta del chip")
	...
	while (true) {
		if ((sw >> 8) == 0x61) {
			...
			if (ln != 0) {
				...
				sw = transmit(VarToByteArray(apdu), &elabresp);
                ODS(std::string().append("\nRESP:").append(dumpHexData(elabresp)).append("\n").c_str());
...
StatusWord IAS::getResp_SM(ByteArray &resp, StatusWord sw, ByteDynArray &elabresp) {
	...
	while (true) {
		if ((sw >> 8) == 0x61) {
			...
			if (ln != 0) {
				uint8_t apdu[] = { 0x00, 0xc0, 0x00, 0x00, ln };
				sw = transmit(VarToByteArray(apdu), &curresp);
				elabresp.append(curresp);
				if (sw == 0x9000)
					break;
				if ((sw >> 8) != 0x61)
...
StatusWord IAS::SendAPDU(ByteArray head, ByteArray data, ByteDynArray &resp, uint8_t *le) {
	...
	if (ds > 255) {
		...
		while (true) {
			...
			StatusWord sw = transmit(apdu, &curresp);
...
//
//    [token tra]
//    BYTE[] resp = transmit(null, apdu.data(), apdu.length());
//
//
//}
